...
class MatchGame extends Component {
  ...
  isSame = id => {
    const {activeImageUrl, score, isRunning} = this.state
  ...
  tick = () => {
    ...
    this.setState(prevState => ({time: parseInt(prevState.time) - 1}))
  }

  renderGame = () => {
    const filteredResults = this.getFilteredResults()
    const {score, activeImageUrl, time, isRunning} = this.state
    if (isRunning) {
      return (
        <div>
          <ul>
  ...
  render() {
    ...
    return (
      <div>
        <div className="bottom-container">{this.renderGame()}</div>
      </div>
    )
  }
}
